...
function startIMAPForAccount(account, config = {}, onNewEmail, folderName = null) {
    ...
    function processNew() {
        ...
        if (effectiveStartUid === null) {
            logger.warn(`⚠️ [${label}] Немає UID для початку пошуку`);
            return;
        }
        ...
        imap.search([['UID', `${effectiveStartUid + 1}:*`]], (err, results) => {
            if (err) {
            ...
            fetch.once('end', async () => {
                ...
                if (maxUidSeen > effectiveStartUid) {
                    startUid = maxUidSeen;
                }
                processing = false;
        ...
        if (!folderName) {
            ...
            imap.getBoxes((err, boxes) => {
                ...
                imap.openBox('INBOX', false, (err, box) => {
                    ...
                    if (storedUid) {
                        ...
                        logger.info(`📭 [${account.user}] INBOX: початковий UID ${startUid} (всього: ${box.messages.total})`);
                    }
            ...
            imap.openBox(folderName, false, (err, box) => {
                ...
                if (storedUid) {
                    ...
                    startUid = box.uidnext - 1;
                    logger.info(`📭 [${label}] початковий UID ${startUid} (всього: ${box.messages.total})`);
                }
