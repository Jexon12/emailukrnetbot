...
function startIMAPForAccount(account, config = {}, onNewEmail, folderName = null) {
    ...
    function processNew() {
        ...
        const storedUid = store.getLastUid(account.user, folderName || 'INBOX');
        const effectiveStartUid = (startUid !== null) ? startUid : storedUid;
        ...
        if (!folderName) {
            ...
            imap.getBoxes((err, boxes) => {
                ...
                imap.openBox('INBOX', false, (err, box) => {
                    ...
                    if (storedUid) {
                        ...
                        // FIX: Default to current highest UID so we don't fetch old unread emails
                        startUid = box.uidnext - 1;
                        logger.info(`📭 [${account.user}] новий акаунт, початок з UID ${startUid} (last 10)`);
                        store.setLastUid(account.user, 'INBOX', startUid);
                    }
            ...
            imap.openBox(folderName, false, (err, box) => {
                ...
                if (storedUid) {
                    ...
                    // FIX: Default to current highest UID so we don't fetch old unread emails
                    startUid = box.uidnext - 1;
                    logger.info(`📭 [${label}] нова папка, початок з UID ${startUid} (last 10)`);
                    store.setLastUid(account.user, folderName, startUid);
                }
...
async function fetchLastEmails(account, limit = 5, config = {}) {
    ...
    return new Promise((resolve, reject) => {
        logger.info(`📥 [${label}] Спроба отримати останні ${limit} листів...`);
        const imap = new Imap({
            user: account.user,
            password: account.password,
            host: account.imapHost || IMAP_HOST,
        ...
        const timeout = setTimeout(() => {
            imap.end();
            reject(new Error('IMAP Timeout (25s)'));
        }, 25000);
        ...
        imap.once('ready', () => {
            imap.openBox('INBOX', true, (err, box) => {
                ...
                if (total === 0) {
                    clearTimeout(timeout);
                    logger.info(`📥 [${label}] Скринька порожня`);
                    imap.end();
                    return resolve([]);
                }

                const start = Math.max(1, total - limit + 1);
                logger.info(`📥 [${label}] Завантажую повідомлення ${start}:${total}...`);
                ...
                fetch.on('message', (msg) => {
                    ...
                    const p = new Promise((res) => {
                        // Individual message timeout (10s)
                        const msgTimeout = setTimeout(() => {
                            logger.warn(`⚠️ [${label}] Message fetch timeout (skipped)`);
                            res();
                        }, 10000);
                        ...
                        msg.on('end', async () => {
                            ...
                            try {
                                ...
                                logger.error(`❌ [${label}] parsing error: ${e.message}`);
                            }
                            res();
                        });
                    });
                ...
                fetch.once('error', (err) => {
                    logger.error(`❌ [${label}] Fetch: ${err.message}`);
                    // Don't reject yet, let 'end' handle it if some emails were fetched
                });

                fetch.once('end', async () => {
                    try {
                        // Wait for all messages with a safe timeout
                        await Promise.race([
                        ...
                        clearTimeout(timeout);
                        imap.end();
                        logger.info(`📥 [${label}] Отримано ${emails.length} листів`);
        ...
        imap.once('error', (err) => {
            clearTimeout(timeout);
            logger.error(`❌ [${label}] IMAP error: ${err.message}`);
            imap.end();
            reject(err);
        });
