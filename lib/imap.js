...
async function fetchLastEmails(account, limit = 5, config = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            imap.end();
            reject(new Error('IMAP Timeout (15s)'));
        }, 20000);
        ...
        imap.once('ready', () => {
            imap.openBox('INBOX', true, (err, box) => {
                ...
                const fetch = imap.fetch(`${start}:*`, { bodies: '' });
                const emails = [];
                const promises = [];
                    ...
                    const p = new Promise((res) => {
                        msg.on('end', async () => {
                            try {
                                ...
                                logger.error(`❌ [${label}] Помилка парсингу: ${e.message}`);
                            }
                            res();
                        });
                    });
                ...
                fetch.once('error', (err) => {
                    logger.error(`❌ [${label}] Помилка fetch: ${err.message}`);
                    imap.end();
                    reject(err);
                });
        ...
        imap.once('error', (err) => {
            clearTimeout(timeout);
            logger.error(`❌ [${label}] Помилка з'єднання: ${err.message}`);
            imap.end();
            reject(err);
        });
        imap.connect();
