...
h1{font-size:28px;font-weight:800;background:linear-gradient(135deg,#667eea,#764ba2);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:8px}
.subtitle{color:#888;font-size:14px;margin-bottom:32px}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin-bottom:32px}
.card{background:linear-gradient(145deg,#1a1a2e,#16213e);border:1px solid #2a2a4a;border-radius:16px;padding:24px;text-align:center;transition:transform .2s,box-shadow .2s}
.card:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(102,126,234,.15)}
.card-value{font-size:36px;font-weight:800;background:linear-gradient(135deg,#667eea,#764ba2);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
...
.panel{background:linear-gradient(145deg,#1a1a2e,#16213e);border:1px solid #2a2a4a;border-radius:16px;padding:24px;margin-bottom:24px}
...
.chart-bar{width:100%;border-radius:6px 6px 0 0;background:linear-gradient(180deg,#667eea,#764ba2);min-height:4px;transition:height .5s}
.chart-label{font-size:11px;color:#888}
.chart-value{font-size:12px;color:#ccc;font-weight:600}
.status-row{display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #2a2a4a}
...
.filter-tag{background:rgba(102,126,234,.15);color:#667eea;padding:4px 12px;border-radius:12px;font-size:13px}
.miniapp-link{display:inline-block;margin-top:16px;padding:12px 24px;background:linear-gradient(135deg,#667eea,#764ba2);color:white;border-radius:12px;text-decoration:none;font-weight:600;font-size:14px;transition:transform .2s}
...
<div class="status-row"><span class="status-label">Фільтри</span><span>${data.filters.length > 0 ? `<div class="filter-list">${data.filters.map(f => `<span class="filter-tag">${f}</span>`).join('')}</div>` : '<span style="color:#888">немає</span>'}</span></div>
</div>
...
${new Date().toLocaleString('uk-UA', { timeZone: 'Europe/Kyiv' })} | <a href="/" style="color:#667eea">🔄</a></p>
</div></body></html>`);
  });
