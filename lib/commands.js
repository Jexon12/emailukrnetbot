...
const { searchEmails, fetchLastEmails } = require('./imap');
...
function registerCommands(bot, config) {
    ...
    bot.onText(/\/status/, (msg) => {
        ...
        bot.sendMessage(msg.chat.id, [
            ...
            reply_markup: {
                inline_keyboard: [[
                    { text: '📥 Останні 5 листів', callback_data: 'last_5' }
                ]]
            }
        });
    });
    ...
    bot.onText(/\/last/, async (msg) => {
        ...
        await bot.sendMessage(msg.chat.id, `📥 Отримую останні 5 листів...`, { parse_mode: 'HTML' });
        ...
        try {
            ...
            if (emails.length === 0) {
                await bot.sendMessage(msg.chat.id, `📭 Скринька порожня`);
            } else {
                // Reuse existing sendToTelegram logic if available, but here we can just format them
                for (const email of emails) {
                    if (config.onNewEmail) {
                        await config.onNewEmail(email);
                    }
                }
                await bot.sendMessage(msg.chat.id, `✅ Готово. Виведено ${emails.length} листів.`);
            }
        } catch (err) {
            logger.error(`❌ /last error: ${err.message}`);
            await bot.sendMessage(msg.chat.id, `❌ Помилка: <code>${esc(err.message)}</code>`, { parse_mode: 'HTML' });
        }
    });
