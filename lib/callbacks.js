...
const { startIMAPForAccount, fetchLastEmails } = require('./imap');
...
function registerCallbacks(bot, config) {
    ...
    bot.on('callback_query', async (query) => {
        ...
        if (data.startsWith('markspam_')) {
            // Format: markspam_{email}_{timestamp} — email may contain underscores
            // Strip prefix and suffix (timestamp after last _)
            const withoutPrefix = data.slice('markspam_'.length);
            const lastUnderscore = withoutPrefix.lastIndexOf('_');
            const sender = lastUnderscore !== -1 ? withoutPrefix.slice(0, lastUnderscore) : withoutPrefix;
        ...
        // Last 5 emails
        if (data === 'last_5') {
            await bot.answerCallbackQuery(query.id, { text: '📥 Отримую 5 листів...' });
            try {
                const account = { user: EMAIL_USER, password: config.EMAIL_PASSWORD, imapHost: config.IMAP_HOST, imapPort: config.IMAP_PORT };
                const emails = await fetchLastEmails(account, 5, config);
                if (emails.length === 0) {
                    await bot.sendMessage(chatId, `📭 Скринька порожня`);
                } else {
                    for (const email of emails) {
                        if (config.onNewEmail) {
                            await config.onNewEmail(email);
                        }
                    }
                }
            } catch (err) {
                logger.error(`❌ last_5 callback error: ${err.message}`);
...
function finishAddMail(bot, chatIdFrom, host, config) {
    ...
    if (state.password) state.password = '';  // wipe before removing reference
